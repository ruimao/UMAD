...
*/
void CTrain_Knn::TrainModel(char *classifyMethod,vector<shared_ptr<CMetricData> > *traindata,vector<string> trainDataLabel, CMetricDistance *metric,CPivotSelectionMethod *pivotselectionmethod, int pivotNum,char *pivotsAndTrainModelFileName,int dim,int coordinate)
{
 ...
 *@ param coordinate the number coordinate of sampling point.
*/
	 void CTrain_Knn::TrainModelUseCrossValidation(char *classifyMethod,vector<shared_ptr<CMetricData> > *traindata,vector<string> trainDataLabel, CMetricDistance *metric,CPivotSelectionMethod *pivotselectionmethod, int pivotNum,char *pivotsAndTrainModelFileName,int dim,int coordinate)
	 {
		 CDatasetInMetricSpace getTrainData;
		 GetMetricData M_traindata;
		 M_traindata=getTrainData.getMetricTrainData(classifyMethod,traindata,trainDataLabel,metric,pivotselectionmethod,pivotNum,pivotsAndTrainModelFileName,dim,coordinate);
		 showClassifierModel_CrossValidation(M_traindata,pivotsAndTrainModelFileName,pivotNum);
	 }
 ...
 *@outfilename: outfilename is going to store the classify result
 */
void CTrain_Knn::showClassifierModel_CrossValidation(GetMetricData M_traindata,const char* outfilename, int pivotNum)
{
	ofstream outfile(outfilename,ofstream::app);  //ofstream::out : create a new file; ofstream::app : open the existing file and add content at the end.
	outfile<<"@trainmodel"<<endl;
	for(auto m=0; m<M_traindata.metricData.size(); ++m) 
